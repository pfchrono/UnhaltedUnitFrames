...
function UUF:CreateTestTargetFrame()
    if InCombatLockdown() then return end
    
    local TargetFrame = UUF.TARGET
    if not TargetFrame then return end
    
    local TargetDB = UUF.db.profile.Units.target
    UUF:ResolveLSM()
    
    if UUF.TARGET_TEST_MODE then
        -- Detach from unit watch to show test data
        TargetFrame:SetAttribute("unit", nil)
        UnregisterUnitWatch(TargetFrame)
        
        if TargetDB.Enabled then TargetFrame:Show() else TargetFrame:Hide() end
        TargetFrame:SetFrameStrata(TargetDB.Frame.FrameStrata)
        
        -- Use boss data index 1 as target test data
        local testData = EnvironmenTestData[1]
        
        if TargetFrame.Health then
            local HealthBarDB = TargetDB.HealthBar
            TargetFrame.Health:SetMinMaxValues(0, testData.maxHealth)
            TargetFrame.Health:SetValue(testData.health)
            TargetFrame.HealthBackground:SetMinMaxValues(0, testData.maxHealth)
            TargetFrame.HealthBackground:SetValue(testData.missingHealth)
            TargetFrame.HealthBackground:SetStatusBarColor(GetTestUnitColour(1, HealthBarDB.Background, HealthBarDB.ColourBackgroundByClass, HealthBarDB.BackgroundOpacity))
            TargetFrame.Health:SetStatusBarColor(GetTestUnitColour(1, HealthBarDB.Foreground, HealthBarDB.ColourByClass, HealthBarDB.ForegroundOpacity))
        end
        
        if TargetFrame.Portrait then
            TargetFrame.Portrait:SetTexture("Interface\\ICONS\\achievement_character_human_female")
        end
        
        if TargetFrame.Name then
            TargetFrame.Name:SetText("Target")
        end
        
        if TargetFrame.Power then
            TargetFrame.Power:SetMinMaxValues(0, testData.maxPower)
            TargetFrame.Power:SetValue(testData.power)
        end
        
        if TargetFrame.Castbar then
            local CastBarDB = TargetDB.CastBar
            local CastBarContainer = TargetFrame.Castbar and TargetFrame.Castbar:GetParent()
            if TargetFrame.Castbar and CastBarDB.Enabled then
                TargetFrame:DisableElement("Castbar")
                CastBarContainer:Show()
                TargetFrame.Castbar:Show()
                TargetFrame.Castbar.Background:Show()
                TargetFrame.Castbar.Text:SetText("Fireball")
                TargetFrame.Castbar.Time:SetText("0.0")
                TargetFrame.Castbar:SetMinMaxValues(0, 1000)
                TargetFrame.Castbar:SetScript("OnUpdate", function() local currentValue = TargetFrame.Castbar:GetValue() currentValue = currentValue + 1 if currentValue >= 1000 then currentValue = 0 end TargetFrame.Castbar:SetValue(currentValue) TargetFrame.Castbar.Time:SetText(string.format("%.1f", (currentValue / 1000) * 3)) end)
                local castBarColour = (false and CastBarDB.NotInterruptibleColour) or (CastBarDB.ColourByClass and UUF:GetClassColour(TargetFrame)) or CastBarDB.Foreground
                TargetFrame.Castbar:SetStatusBarColor(castBarColour[1], castBarColour[2], castBarColour[3], castBarColour[4])
                if CastBarDB.Icon.Enabled and TargetFrame.Castbar.Icon then TargetFrame.Castbar.Icon:SetTexture("Interface\\Icons\\spell_fire_fireball") TargetFrame.Castbar.Icon:Show() end
            else
                if CastBarContainer then CastBarContainer:Hide() end
                if TargetFrame.Castbar and TargetFrame.Castbar.Icon then TargetFrame.Castbar.Icon:Hide() end
            end
        end
        -- Ensure frame stays visible in test mode
        if TargetDB.Enabled then TargetFrame:Show() end
    else
        -- Re-attach to unit watch
        TargetFrame:SetAttribute("unit", "target")
        RegisterUnitWatch(TargetFrame)
        TargetFrame:Hide()
        
        if TargetFrame.UpdateAllElements then
            TargetFrame:UpdateAllElements("RefreshUnit")
        end
    end
end

function UUF:CreateTestFocusFrame()
    if InCombatLockdown() then return end
    
    local FocusFrame = UUF.FOCUS
    if not FocusFrame then return end
    
    local FocusDB = UUF.db.profile.Units.focus
    UUF:ResolveLSM()
    
    if UUF.FOCUS_TEST_MODE then
        -- Detach from unit watch to show test data
        FocusFrame:SetAttribute("unit", nil)
        UnregisterUnitWatch(FocusFrame)
        
        if FocusDB.Enabled then FocusFrame:Show() else FocusFrame:Hide() end
        FocusFrame:SetFrameStrata(FocusDB.Frame.FrameStrata)
        
        -- Use boss data index 2 as focus test data
        local testData = EnvironmenTestData[2]
        
        if FocusFrame.Health then
            local HealthBarDB = FocusDB.HealthBar
            FocusFrame.Health:SetMinMaxValues(0, testData.maxHealth)
            FocusFrame.Health:SetValue(testData.health)
            FocusFrame.HealthBackground:SetMinMaxValues(0, testData.maxHealth)
            FocusFrame.HealthBackground:SetValue(testData.missingHealth)
            FocusFrame.HealthBackground:SetStatusBarColor(GetTestUnitColour(2, HealthBarDB.Background, HealthBarDB.ColourBackgroundByClass, HealthBarDB.BackgroundOpacity))
            FocusFrame.Health:SetStatusBarColor(GetTestUnitColour(2, HealthBarDB.Foreground, HealthBarDB.ColourByClass, HealthBarDB.ForegroundOpacity))
        end
        
        if FocusFrame.Portrait then
            FocusFrame.Portrait:SetTexture("Interface\\ICONS\\achievement_character_human_male")
        end
        
        if FocusFrame.Name then
            FocusFrame.Name:SetText("Focus")
        end
        
        if FocusFrame.Power then
            FocusFrame.Power:SetMinMaxValues(0, testData.maxPower)
            FocusFrame.Power:SetValue(testData.power)
        end
        
        if FocusFrame.Castbar then
            local CastBarDB = FocusDB.CastBar
            local CastBarContainer = FocusFrame.Castbar and FocusFrame.Castbar:GetParent()
            if FocusFrame.Castbar and CastBarDB.Enabled then
                FocusFrame:DisableElement("Castbar")
                CastBarContainer:Show()
                FocusFrame.Castbar:Show()
                FocusFrame.Castbar.Background:Show()
                FocusFrame.Castbar.Text:SetText("Holy Light")
                FocusFrame.Castbar.Time:SetText("0.0")
                FocusFrame.Castbar:SetMinMaxValues(0, 1000)
                FocusFrame.Castbar:SetScript("OnUpdate", function() local currentValue = FocusFrame.Castbar:GetValue() currentValue = currentValue + 1 if currentValue >= 1000 then currentValue = 0 end FocusFrame.Castbar:SetValue(currentValue) FocusFrame.Castbar.Time:SetText(string.format("%.1f", (currentValue / 1000) * 2.5)) end)
                local castBarColour = (false and CastBarDB.NotInterruptibleColour) or (CastBarDB.ColourByClass and UUF:GetClassColour(FocusFrame)) or CastBarDB.Foreground
                FocusFrame.Castbar:SetStatusBarColor(castBarColour[1], castBarColour[2], castBarColour[3], castBarColour[4])
                if CastBarDB.Icon.Enabled and FocusFrame.Castbar.Icon then FocusFrame.Castbar.Icon:SetTexture("Interface\\Icons\\spell_holy_holylight") FocusFrame.Castbar.Icon:Show() end
            else
                if CastBarContainer then CastBarContainer:Hide() end
                if FocusFrame.Castbar and FocusFrame.Castbar.Icon then FocusFrame.Castbar.Icon:Hide() end
            end
        end
        -- Ensure frame stays visible in test mode
        if FocusDB.Enabled then FocusFrame:Show() end
    else
        -- Re-attach to unit watch
        ...
        if FocusFrame.UpdateAllElements then
            FocusFrame:UpdateAllElements("RefreshUnit")
        end
    end
end

function UUF:CreateTestPetFrame()
    if InCombatLockdown() then return end
    
    local PetFrame = UUF.PET
    if not PetFrame then return end
    
    local PetDB = UUF.db.profile.Units.pet
    UUF:ResolveLSM()
    
    if UUF.PET_TEST_MODE then
        -- Detach from unit watch to show test data
        PetFrame:SetAttribute("unit", nil)
        UnregisterUnitWatch(PetFrame)
        
        if PetDB.Enabled then PetFrame:Show() else PetFrame:Hide() end
        PetFrame:SetFrameStrata(PetDB.Frame.FrameStrata)
        
        -- Use boss data index 3 as pet test data
        local testData = EnvironmenTestData[3]
        
        if PetFrame.Health then
            local HealthBarDB = PetDB.HealthBar
            PetFrame.Health:SetMinMaxValues(0, testData.maxHealth)
            PetFrame.Health:SetValue(testData.health)
            PetFrame.HealthBackground:SetMinMaxValues(0, testData.maxHealth)
            PetFrame.HealthBackground:SetValue(testData.missingHealth)
            PetFrame.HealthBackground:SetStatusBarColor(GetTestUnitColour(3, HealthBarDB.Background, HealthBarDB.ColourBackgroundByClass, HealthBarDB.BackgroundOpacity))
            PetFrame.Health:SetStatusBarColor(GetTestUnitColour(3, HealthBarDB.Foreground, HealthBarDB.ColourByClass, HealthBarDB.ForegroundOpacity))
        end
        
        if PetFrame.Portrait then
            PetFrame.Portrait:SetTexture("Interface\\ICONS\\achievement_character_dwarf_male")
        end
        ...
        if PetFrame.Power then
            PetFrame.Power:SetMinMaxValues(0, testData.maxPower)
            PetFrame.Power:SetValue(testData.power)
        end
        
        if PetFrame.Castbar then
            local CastBarDB = PetDB.CastBar
            local CastBarContainer = PetFrame.Castbar and PetFrame.Castbar:GetParent()
            if PetFrame.Castbar and CastBarDB.Enabled then
                PetFrame:DisableElement("Castbar")
                CastBarContainer:Show()
                PetFrame.Castbar:Show()
                PetFrame.Castbar.Background:Show()
                PetFrame.Castbar.Text:SetText("Bite")
                PetFrame.Castbar.Time:SetText("0.0")
                PetFrame.Castbar:SetMinMaxValues(0, 1000)
                PetFrame.Castbar:SetScript("OnUpdate", function() local currentValue = PetFrame.Castbar:GetValue() currentValue = currentValue + 1 if currentValue >= 1000 then currentValue = 0 end PetFrame.Castbar:SetValue(currentValue) PetFrame.Castbar.Time:SetText(string.format("%.1f", (currentValue / 1000) * 1.5)) end)
                local castBarColour = (false and CastBarDB.NotInterruptibleColour) or (CastBarDB.ColourByClass and UUF:GetClassColour(PetFrame)) or CastBarDB.Foreground
                PetFrame.Castbar:SetStatusBarColor(castBarColour[1], castBarColour[2], castBarColour[3], castBarColour[4])
                if CastBarDB.Icon.Enabled and PetFrame.Castbar.Icon then PetFrame.Castbar.Icon:SetTexture("Interface\\Icons\\ability_pet_bite") PetFrame.Castbar.Icon:Show() end
            else
                if CastBarContainer then CastBarContainer:Hide() end
                if PetFrame.Castbar and PetFrame.Castbar.Icon then PetFrame.Castbar.Icon:Hide() end
            end
        end
        -- Ensure frame stays visible in test mode
        if PetDB.Enabled then PetFrame:Show() end
    else
        -- Re-attach to unit watch
        ...
        if PetFrame.UpdateAllElements then
            PetFrame:UpdateAllElements("RefreshUnit")
        end
    end
end
